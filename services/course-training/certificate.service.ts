import { IGetCertificateByCourseIdService, IGetCertificateServiceRes } from './types'
...
export const getCertificatesService = async () => {
  const response = await axiosInstance.get(`${baseUrl}/certificates`)
  const res: IGetCertificateServiceRes = response.data
