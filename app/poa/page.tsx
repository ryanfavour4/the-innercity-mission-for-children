...
import WishCarousel, { Testimonial } from './sections/birthday-wish-wall'
...
const links = [
  { name: 'ROAD TO POA630 “26', href: '#road' },
  { name: 'GALLERY', href: '#gallery' },
  { name: 'BIRTHDAY WISH WALL', href: '#birthday-wall' },
]
...
export default function Poa() {
  ...
  return (
    <main className="font-playfair-display min-h-screen bg-[#FAF6EE] leading-none">
        ...
        <section className="mx-auto h-fit w-full pt-16 xl:container md:grid md:grid-cols-12">
          ...
          >
            <figcaption className="absolute bottom-0 right-0 flex w-full flex-col gap-1 bg-gradient-to-b from-dark/10 via-dark/90 to-dark px-4 pb-10 md:static md:w-auto md:gap-2 md:from-transparent md:via-transparent md:to-transparent md:pb-8">
              <span className="w-fit border border-[#C9A84C] bg-[#2A1A1F]/50 px-2 py-1 text-[0.5rem] uppercase tracking-wide text-[#C9A84C] md:mb-5 md:text-xs">
                executive minister - MINISTRY OF the innercity missions for CHILDREN
              </span>

              <h2 className="font-playfair-display text-balance text-3xl md:text-5xl">
                Highly Esteemed Pastor Omoh Alabi
              </h2>

              <hr className="my-4 max-w-[8rem] border border-[#C9A84C]" />

              <blockquote className="mb-2 max-w-sm text-base italic md:mb-6 md:text-xl">
                &quot;Demonstrating God’s love through service, leadership and unwavering commitment
                to advancing the vision and impacting lives&quot;
              </blockquote>
              ...
              >
                {[
                  { name: 'ROAD TO POA630 “26', link: '#road' },
                  { name: 'GALLERY', link: '#gallery' },
                  { name: 'SEND BIRTHDAY GREETINGS', link: '#send-wishes' },
                ].map((text, i) => (
                  <motion.button
                    key={text.name}
                    variants={{
                    ...
                    className={`w-fit text-sm tracking-wide md:text-base ${
                      i === 0
                        ? 'bg-[#B38945] text-[#2b210f] hover:bg-[#B38945]/80'
                        : 'border-2 border-[#B38945] text-light hover:bg-[#B38945]/25'
                    }`}
                  >
                    <a href={text.link} className="block px-7 py-3">
                      {text.name}
                    </a>
                  </motion.button>
                ))}
      ...
      <section className="bg-[#1C1C17] py-4 pb-6 text-light">
        <h6 className="container mb-3 px-3 text-xs font-semibold text-[#C9A84C] md:text-sm">
          BIRTHDAY GREETINGS
        </h6>

        <Marquee>
          <div className="flex items-center justify-center gap-8">
            {testimonials?.map((wish) => (
              <article key={wish.id} className="justify-conter flex items-center gap-3">
                <q className="italic">{wish.message}</q>
                <p>- {wish.name}</p>
              </article>
            ))}
          </div>
      ...
      <section id="road" className="py-14">
        <div className="container px-3">
          <motion.h3
            ...
            variants={fadeUp}
            className="font-playfair-display mx-auto mb-10 max-w-[11rem] text-center text-[2.8rem] font-medium italic leading-normal text-[#B38945] md:max-w-max md:text-[4rem]"
          >
            Road To POA630 ‘’26
          </motion.h3>
      ...
      <section>
        ...
        >
          <div className="font-playfair-display flex flex-col items-center justify-center gap-2 text-center">
            <motion.h3
              ...
              className="font-playfair-display text-5xl text-[#F1F1F1]"
            >
              HER STORY
            </motion.h3>
        ...
        <div className="container pt-24">
          <motion.h3
            ...
            variants={fadeUp}
            className="font-playfair-display mx-auto mb-10 max-w-sm text-center text-[2.8rem] font-medium italic leading-normal text-[#B38945] md:max-w-max md:text-[4rem]"
          >
      ...
      <section className="bg-[#B38945] py-10 text-light md:py-20">
        <div className="container flex grid-cols-2 flex-col-reverse gap-5 px-3 py-10 md:grid">
          ...
          <motion.span
            ...
            className="-mt-52 flex w-full flex-col items-center justify-center gap-5 bg-[#1C1C17] px-16 py-16 text-center"
          >
            <motion.small
              initial={{ opacity: 0 }}
      ...
      <section id="gallery" className="bg-poa-gallery bg-cover py-10 text-light md:py-16">
        <div className="container mx-auto flex max-w-7xl flex-col gap-6 px-3 md:gap-y-10">
          <TabbedGallery />
        </div>
      </section>

      <section id="birthday-wall" className="pb-16">
        <div className="font-playfair-display bg-poa-hero mb-10 flex flex-col items-center justify-center gap-2 overflow-hidden px-3 py-28 text-center text-light">
          <h3 className="font-playfair-display text-5xl text-[#B38945]">BIRTHDAY WISH WALL</h3>
          ...
          <span className="flex w-full items-center justify-center gap-2 text-xs md:text-base">
            <hr className="my-4 w-full max-w-[2rem] border-[#B38945]" />
      ...
      <footer className="font-playfair-display bg-poa-her flex flex-col items-center justify-center gap-y-10 overflow-hidden bg-[#1C1C17] px-3 py-28 pb-20 text-center text-light">
        <h3 className="font-playfair-display text-2xl text-[#C9A84C] md:text-3xl">
          Celebrating An Icon
        </h3>
        ...
        <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-4 text-sm">
          {links.map((link) => (
            <li key={link.name}>
              <a
                href={link.href}
                className="style-1-nav-link tracking-wider after:!bg-[#F1F1F1] hover:text-[#F1F1F1]"
              >
                {link.name}
              </a>
            </li>
...
function Navbar() {
  ...
  return (
    <>
      <nav className="border-b border-[#F1F1F1]/50 bg-[#B38945] px-8 py-4">
        <div className="container flex items-center justify-between text-white">
          <span className="flex flex-col items-start gap-1 text-[#F1F1F1]">
            <h1 className="font-playfair-display text-xl md:text-2xl">Celebrating An Icon</h1>
          ...
          >
            <span className="flex flex-row items-center justify-center px-2 text-sm text-[#F1F1F1] after:!bg-[#F1F1F1] hover:text-[#F1F1F1] focus:outline-none md:hidden">
        ...
        >
          <main
            className={`${mobileMenuOpen ? 'delay-200 clip-path-slide-top-down' : 'clip-path-close'} h-full w-full overflow-y-auto bg-[#B38945] px-8 py-4 text-light duration-500`}
          >
            ...
            <div className="flex h-[85%] flex-col items-center justify-center">
              <ul className="mt-16 flex w-full max-w-sm flex-col items-start gap-8 text-lg">
                {links.map((link) => (
                  <li onClick={() => setMobileMenuOpen(!mobileMenuOpen)} key={link.name}>
                    <a
                      href={link.href}
                      className="style-1-nav-link tracking-widest after:!bg-[#F1F1F1] hover:text-[#F1F1F1]"
                    >
                      {link.name}
                    </a>
                  </li>
              ...
              <figure className="relative -mr-16 mt-auto block h-32 w-svw overflow-hidden rounded-l-full border-2 border-[#F1F1F1]">
