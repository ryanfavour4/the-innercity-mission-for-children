...
const VideoPlayer = ({ src, thumb = thumbnail, autoplay = false, className }: VideoPlayerProps) => {
  ...
  const togglePlay = (e?: React.MouseEvent) => {
    ...
    setPlaying((prev) => !prev)
    if (!playing) setIsHovering(true) // Show controls when pausing
  }
  // ----------------------------
  ...
  const handleJumpPlayerCurrentTime = (e: React.MouseEvent, jumpBy: number) => {
    e.stopPropagation() // Prevent the container click from firing
    if (!playerRef.current || !jumpBy) return
    playerRef.current.currentTime = playerRef.current.currentTime + jumpBy
  }
  ...
  const handleVideoEnded = () => {
    ...
    setProgress(0)
    setShowThumbnail(true)
    setIsHovering(true) // Show controls when video ends
  }
  ...
  return (
    <>
      <div
        // NEW: Click to toggle visibility (Mobile) and mouse move to show (Desktop)
      ...
      >
        ...
        {thumb && (
          <Image
            alt="thumbnail"
            src={thumb}
            className={`absolute inset-0 z-[1] h-full w-full object-cover ${showThumbnail ? 'block' : 'hidden'}`}
            width={640}
            height={360}
          />
        )}
        ...
        <div
          onMouseEnter={() => setIsHovering(true)}
          onMouseLeave={() => setIsHovering(false)}
          // NEW: Added pointer-events-none when hidden so it doesn't block clicks
          className={`absolute inset-0 z-10 bg-black/20 text-light transition-opacity duration-500 ${isHovering ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
        >
          ...
          >
            <button className="rounded-full bg-white/20 p-4 backdrop-blur-md">
              {playing ? (
          ...
          <div className="absolute bottom-0 h-16 w-full">
            <div
              onClick={(e) => e.stopPropagation()} // Stop bar clicks from hiding the UI
              className="mx-auto h-14 rounded-lg bg-white/30 px-2 py-1.5 backdrop-blur-sm md:w-11/12"
            >
              <PercentageBar
                value={progress * 100}
                onChange={(val) => {
              ...
              <div className="mt-1.5 flex items-center justify-between gap-x-8 gap-y-2 text-sm leading-tight md:flex-nowrap">
                ...
                <div className="flex w-full items-center justify-end gap-2">
                  <button
                    onClick={(e) => handleJumpPlayerCurrentTime(e, -10)}
                    className="rounded border border-light bg-light/25 p-1 text-light backdrop-blur active:scale-95"
                  >
                    <SkipBackward10Icon className="size-4 text-base" />
                  </button>
